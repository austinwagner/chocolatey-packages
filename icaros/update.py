...
def find_href(soup: BeautifulSoup, regex: str) -> str:
...
def find_release_notes(soup: BeautifulSoup, version: Version, release_kind: ReleaseKind) -> str:
    version = version.truncate('minor')
    changelog = soup.find(id='changelog').find_next_sibling('div', class_='row0').text.strip()
    ...
    release_notes = ''
    for match, section in split_by_regex(r'.*Icaros v\.(?P<version>[\w\.]+)\s+(?P<kind>[\w ]+)', changelog):
        if Version(match["version"]).truncate('minor') == version and kind_matches(match["kind"]):
...
def update_package(release_kind: ReleaseKind, match: re.Match, latest_version: Version):
    ...
    if latest_version > curr_version:
        ...
        if not prompt_yn('Publish with this nuspec?', 'n'):
            print(f"Fix the nuspec file manually then run 'choco pack' and 'choco push {nupkg_name} --source {PACKAGE_REPO}'")
            return
        ...
        subprocess.check_call(['choco', 'push', nupkg_name, '--source', PACKAGE_REPO], cwd=release_path)
